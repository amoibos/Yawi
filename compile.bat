...
if %TARGET_PLATFORM% == "SG"  		set command=%compiler% %FLAGS% -o %output%.ihx -mz80 --no-std-crt0 --data-loc 0xC000 crt0/crt0_sg.rel  city.rel font.rel levels.rel console.rel strings.rel engine.rel views.rel widgets.rel %mainentry%.rel src/SGlib.rel
if %TARGET_PLATFORM% == "SC"  		set command=%compiler% %FLAGS% -o %output%.ihx -mz80 --no-std-crt0 --data-loc 0xC000 crt0/crt0_sg.rel  city.rel font.rel levels.rel console.rel strings.rel engine.rel views.rel widgets.rel %mainentry%.rel src/SGlib.rel    
